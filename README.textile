...
h2. Options
* speed (default 200) => Accepts standard jQuery effects speeds (i.e. fast, normal or milliseconds)
* direction (default right) => Accepts 'left' or 'right'
* modal (default false) => If set to true, you must explicitly close pageslide using $.pageslide.close();
* iframe (default true) => By default, linked pages are loaded into an iframe. Set this to false if you don't want an iframe.
* href (default null) => Override the source of the content. Optional in most cases, but required when opening pageslide programmatically.
* disableRefresh (default false) => is the content is refreshed each time we open the pageslide

h2. Changelog

h3. Version 2.1
* Add an option to disable the refresh of the content each time we open it
